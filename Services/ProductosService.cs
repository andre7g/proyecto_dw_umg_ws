...
namespace umg_clinica_backend.Services
{
    public class ProductosService : ProductosInterface
    {
        ...
        {
            IEnumerable<Productos> list = _context.Productos.Include(x => x.Via_Administracion_Producto).Include(x => x.Marca).Include(x => x.Dosis).Include(x => x.Presentacion).ToList();
            Response res = new Response();
            if (list.Count() != 0)
            {
                res.Status = 200;
